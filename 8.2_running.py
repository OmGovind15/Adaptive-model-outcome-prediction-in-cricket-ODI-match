...
train_folder = r"C:\Users\91878\Documents\projects\New folder\odis_male_csv2\innings2\train_processed"
test_folder = r"C:\Users\91878\Documents\projects\New folder\odis_male_csv2\innings2\test_processed"
...
def feature_engineering(data):
    ...
    data['required_run_rate'] = data['trail_runs'] / (50-(data['over']))
    ...
    data['pressure_index'] = (300*(data['required_run_rate']-data['run_rate'])) / ((data['wickets_remaining'] + 1)*(data['momentum_factor']+1)*(data['balls_remaining']+1))
    ...
    features = ['trail_runs', 'toss_result', 'wickets_remaining', 'bowling_team_win_percentage',
                'rate_gap', 'pressure_index', 'momentum_factor', 'weighted_batting_average', 'weighted_bowling_average']
...
results_df.to_excel('model_details_2.xlsx', index=False)

# Path to Excel file
excel_path = "model_details_2.xlsx"
...
for over in train_data['over'].unique():
    ...
    for model_name in models.keys():
        ...
        # CV Score (mean of the validation accuracies)
        if over_index < len(validation_accuracies[model_name]):
            row[f'{model_name}_CV_Score'] = sum(validation_accuracies[model_name][over_index]) / len(validation_accuracies[model_name][over_index])  # Mean
        else:
            row[f'{model_name}_CV_Score'] = None

        # Test accuracy (assuming it's recorded per over, not per fold)
        if over_index < len(model_accuracies[model_name]):
            row[f'{model_name}_Test_Accuracy'] = model_accuracies[model_name][over_index]
        else:
            row[f'{model_name}_Test_Accuracy'] = None
    ...
    # Best model and its test accuracy
    row['Best_Model_Selected'] = model_usage.get(over, "Unknown")
    row['Best_Model_Test_Accuracy'] = (
        best_model_accuracies[over_index] if over_index < len(best_model_accuracies) else None
    )
